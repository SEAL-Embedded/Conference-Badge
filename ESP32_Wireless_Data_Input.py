...
def show_rssi_color(rssi, matched):
    ...
    set_rgb(r_col * brightness, g_col * brightness, y_col * brightness)
...
async def main():
    # unzips file created with bash
    with open("config.json", "r") as f:
        config = json.load(f)

    device_id = config["device_id"]
    RESEARCH_FIELD = config["research_field"]
    HOME_INSTITUTION = config["home_institution"]
    COMPANY_AFFILIATION = config["company_affiliation"]
    NAME = config["name"]
    # [RESEARCH_FIELD, HOME_INSTITUTION, COMPANY_AFFILIATION]
    badge = Badge([RESEARCH_FIELD, HOME_INSTITUTION, COMPANY_AFFILIATION], [RESEARCH_FIELD, HOME_INSTITUTION, COMPANY_AFFILIATION], 1, NAME)
