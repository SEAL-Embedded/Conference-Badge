...
#------ this set of already connected is not used currently 
        ...
        self.connected_passively = asyncio.Event()
        #self.connection_found = asyncio.Event()
        
        #this should be looked over, but initially debugging 
        self.addr = None
        ...
        async with aioble.scan(1000, interval_us=30000, window_us=20000, active=True) as scanner:
            async for result in scanner:
                if _BADGE_SERVICE_UUID in result.services():
                    #print(f"result.device type: {type(result.device)}")     # debugging
#------------------ remove and if anything
                    if not (result.device in self.already_connected) and not (self.connected_passively.is_set()):

                        print(f"Found device: {result.name()} RSSI: {result.rssi} Address: {result.device}")
                        print()

                        #if the manufacturer info is found
                        try:
                            ...
                            if not manufacturer_list:
                                print("Manufacturer list was found, but it is empty")
                                continue        #next iteration of scanning
                            ...
                            if is_tracking:
                                ...
                                print()
                                continue        #go to the next iteration without adding this to the already connected
                            
                            #this whole thing should not be touched!!! from here
                        ...
                        #if their tags are not what I am looking for
                        if not self.check_match(read_info):
                            continue
                        
                        else:
                            #if my tags are not something they are looking for
#-------------------------- this probably should be discussed with Professor, or this function should be turned off at some point
                            if not self.check_IAM_match(read_target):
                                continue

                            print("Found a good match on both sides! ")     #should flash something to indicate? 
                            self.good_match.set()           #set the good_match event
                    ...
                    elif (self.connected_passively.is_set()):
                        await asyncio.sleep_ms(500)
                        print("Connection was made passively")
                        return True
...
#---------------------- do something with it?
        ...
        print("Another iteration of the scanning loop is done without finding a connection")
        return None
...
#-------------- should flash something to indicate? (for trials without switch)
                ...
                self.connected_passively.set()
                
                #don't touch that!!! from here
    ...
    def check_match(self, read_info):
        ...
        if match >= 2:
            print("This is a person you are looking for!")
            return True
        else:
    ...
    def check_IAM_match(self, read_target):
        ...
        if match >= 2:
            print("You are the person they are looking for!")
            return True
        else:
...
#--------------------------------------------- RSSI values also here
    async def distance_feedback(self):
        if self.is_tracking and (self.current_rssi is not None) and not (self.connection_made.is_set()):
...
#-------------- These values are not right
    ...
    async def search_with_scan(self, addr, timeout_s):
        ...
        self.connected_passively.clear()      #clear the event for searching
...
#-------------------------- protection was removed -------------------------------
                            ...
                            #kinda want to just turn on the relevant lights, no flickering
                            await self.distance_feedback()
