...
class Badge:
            ...
            async with aioble.scan(_SCAN_CYCLE_MS, interval_us, window_us, active=True) as scanner:
                async for result in scanner:
                    ...
                    if reciprocated_id is not None and reciprocated_id == best_id:
                        ...
                        self.device_addr_scan = self._extract_mac_address(reciprocated_device)
    ...
    async def advertise(self):
        while True:  
            ...
            manufacturer_data = tracking_byte + tolerance_byte + color_byte + self.ack_target + self.adv_self + self.adv_search 
...
#---------- lets look at the "tracking"
            if self.is_tracking and self.current_rssi is not None: 
                ...
                #led.off()  #this is the on-board led
                rgb_off()
                await asyncio.sleep_ms(100)
    ...
    async def run_task(self):
        ...
        while True:
            ...
            #one should get address, but a bit differently
